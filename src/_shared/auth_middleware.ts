...
import { redis_token, redis_url, supabase_publishable_key, supabase_base_url } from './config.js';
...
export async function authenticateRequest(bearerKey: string): Promise<AuthResult> {
  ...
  if (credentials) {
    ...
    const userIdFromRedis = await redis.get(email);
    if (!userIdFromRedis) {
      ...
      if (data.user.role !== 'authenticated') {
        ...
        await redis.setex(email, 3600, data.user.id);
        return {
