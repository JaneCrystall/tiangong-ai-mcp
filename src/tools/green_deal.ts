...
const input_schema = {
  ...
  filter: z
    .object({
      ...
      document_id: z.array(z.string()).optional().describe('Filter by document identifier.'),
      issue_agency: z.array(z.string()).optional().describe('Filter by issuing agency.'),
      tags: z.array(z.string()).optional().describe('Filter by Green Deal tags.'),
...
async function searchGreenDeal(
  ...
  const url = `${supabase_base_url}/functions/v1/green_deal_search`;
  try {
